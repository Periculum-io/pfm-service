...
def analyse_transctions(data, salary_variables, other_income_variables, account_name):    
    ...
    try:
        ...
        if len(data) == 0:
            ...
            if len(debit_transactions) == 0:
                ...
                summary["total_outflow"] = round(float(data[data["type"] == "debit"]["amount"].sum()), 2)
        ...
        output = {"status": "success", "output": summary}

        return output
    except Exception as e:
        traceback.print_exc()
        return {"status":"error", "output": "An errror occured while processing your request" }
