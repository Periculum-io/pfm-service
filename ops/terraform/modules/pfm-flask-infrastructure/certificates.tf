...
data "aws_acm_certificate" "acm_certificate_pfm" {
  domain   = var.ec2_lb_domain_name
  statuses = ["ISSUED"]
}
...
resource "aws_route53_record" "certificate_validation_record" {
  for_each = {
    for dvo in data.aws_acm_certificate.acm_certificate_pfm.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
