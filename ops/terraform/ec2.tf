...
data "aws_subnets" "insights_public_subnets" {
  filter {
    name    = "subnet-id"
    values  = var.vpc_insights_public_subnets
  }
}

data "aws_subnet" "insights_public_subnet" {
  for_each  = toset(data.aws_subnets.insights_public_subnets.ids)
  id        = each.value
}
...
resource "aws_security_group_rule" "security_group_rule_ec2_http_ingress" {
  ...
  source_security_group_id  = aws_security_group.security_group_pdf_parser_alb.id
}
